...
export default {
    ...
    computed: {
        previewSourceCode() {
            return this.sourceCode.replace(/'\.\.\/\.\.\/index'/g, `'@tencent/gpxs-ui'`);
        },
    },
    async mounted() {
        if (this.compName && this.demoName) {
            if (isDev) {
                ...
                this.sourceCode = await fetch(`${isDev ? '' : '/gpxs-ui'}/packages/${this.compName}/docs/${this.demoName}.vue`).then((res) => res.text());
            }
        }
        await this.$nextTick();
        Prism.highlightAll();
